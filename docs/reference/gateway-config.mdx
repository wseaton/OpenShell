...
enabled = true
volume_name = "openshell-logs"
mount_path = "/var/log/openshell"

[openshell.drivers.kubernetes.log_collection.sidecar]
enabled = true
name = "openshell-log-collector"
...
`pod_extensions` is an operator-owned Kubernetes-only pod extension surface. It supports named `empty_dir`, `config_map_name`, or `secret_name` volumes; agent volume mounts; and sidecar containers with env, command, args, image pull policy, resources, and volume mounts. Extension mounts cannot use reserved OpenShell control paths such as `/opt/openshell`, `/etc/openshell`, `/etc/openshell-tls`, `/run/netns`, `/var/run/secrets/openshell`, or `/sandbox`.

`log_collection` creates a shared log volume, mounts it into the agent, sets `OPENSHELL_LOG_DIR`, and optionally adds a collector sidecar mounted read-only on that log volume. The log collector sidecar can mount additional `pod_extensions.volumes`, for example a ConfigMap containing collector configuration.
